...
module.exports = DropDownMenu = (function() {
  function DropDownMenu(el) {
    this.onClickItem = bind(this.onClickItem, this);
    this.$el = $(el);
    this.$btn = this.$el.find('.btn');
    this.$itemList = this.$el.find('.itemList');
    this.$item = this.$el.find('.item');
    this.$btn.on('click', (function(_this) {
      return function(e) {
        _this.$itemList.toggleClass('isHidden');
        return e.stopPropagation();
      };
    })(this));
    $(window).on('click', (function(_this) {
      return function() {
        if (!_this.$itemList.hasClass('isHidden')) {
          return _this.$itemList.addClass('isHidden');
        }
      };
    })(this));
    this.$item.each((function(_this) {
      return function(idx, el) {
        return $(el).on('click', _this.onClickItem);
      };
    })(this));
  }

  DropDownMenu.prototype.onClickItem = function(e) {
    var $el;
    e.returnValue = false;
    if (e.preventDefault) {
      e.preventDefault();
